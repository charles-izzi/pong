        ...
        <v-dialog v-model="show" max-width="350">
            <v-card>
                ...
                <v-card-text class="text-left subtitle-1">
                    Use player selection to start a new match-making session or continue the existing one?
                    <v-tooltip bottom>
                        ...
                        <v-card-text
                            class="text-left"
                        >Select 'Continue Existing' if you're using the player selection to change the player pool in the existing match-making session</v-card-text>
                        <v-card-text
                            class="text-left"
                        >Select 'Start New' to forget the existing match-making session and use the player selection to start a new one</v-card-text>
                    </v-tooltip>
                </v-card-text>
...
@Component({
    ...
    watch: {
        show: function(val) {
            if (val && (!this.$store.state.matchMaker.playerPool || !Object.keys(this.$store.state.matchMaker.playerPool).length))
