<template>
  <v-dialog v-model="$store.state.showAddDialog" persistent max-width="600px">
    <template></template>
    <v-card>
      <v-card-title>
...
export default class AddPlayer extends Vue {
  ...
  close() {
    ...
    this.$store.dispatch("setShowAddDialog", false);
  }
  async saveAndClose() {
    if ((this.$refs.addForm as any).validate()) {
      this.$store.dispatch("setShowAddDialog", false);
      await this.$store.dispatch("addNewUser", this.name);
