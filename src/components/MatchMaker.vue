...
export default class MatchMaker extends Vue {
    ...
    get matches() {
        ...
        const matches = this.$store.state.matchMaker.getMatches() as IMatch[];
    ...
    upNextClick(val: IMatch) {
        this.overlay = !this.overlay;
        this.swapMatch = val;
    }
    swapActive(val: IMatch) {
        if (!this.overlay) return;
        this.$store.state.activeMatches.splice(this.$store.state.activeMatches.indexOf(val), 1, this.swapMatch);
