    ...
    <div>
        <v-dialog v-model="$store.state.play.showDialog" :scrollable="false" max-width="500px">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="gray darken-1" text @click="$store.dispatch('setShowPlayDialog', false)">Close</v-btn>
                    <v-btn
                        class="ps-3"
            ...
            <v-card>
                ...
                <v-simple-table>
                    <template v-slot:default>
                        ...
                        <tbody>
                            <tr v-for="(id) in [$store.state.play.player1, $store.state.play.player2]" :key="id">
                                <td>{{ $store.getters.player(id).player }}</td>
                                <td :style="{color: !$store.getters.isPlayerWinner(id) ? 'red' : 'green'}">
