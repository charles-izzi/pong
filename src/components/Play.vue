  ...
  <div>
    <v-dialog v-model="$store.state.showPlayDialog" :scrollable="false" max-width="500px">
      <v-card>
        ...
        <v-row>
          <v-col class="px-10">
            <player :id="player1Id" :playing="true" :name="player1.player" :elo="player1.elo"></player>
          </v-col>
        </v-row>
        <v-row>
          <v-col class="px-10">
            <player :id="player2Id" :playing="true" :name="player2.player" :elo="player2.elo"></player>
          </v-col>
        </v-row>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="gray darken-1" text @click="close()">Close</v-btn>
          <v-btn class="ps-3" color="green darken-1" text :disabled="!$store.state.winner" @click="completeMatch()">Confirm</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
import { IPlayer } from "../business/playModel";
...
export default class Play extends Vue {
  ...
  showNegative(ix: number) {
    return (!this.player1Wins && ix === 0) || (this.player1Wins && ix === 1);
  }
  async completeMatch() {
