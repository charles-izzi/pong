...
import {
  ...
  IMatchesFilter,
  IPlayDialog
} from "@/business/playModel";
import { MatchMaker } from "@/business/matchMaking/matchMaker";
import { Match as MatchMakerMatch } from "@/business/matchMaking/match";
...
export default new Vuex.Store({
  state: {
    ...
    activeMatches: [] as MatchMakerMatch[]
  },
  getters: {
    players: state => {
    ...
    playerByName: state => (name: string) => {
      return Object.keys(state.players || {}).find(
        x => state.players[x].player.toLowerCase() === name.toLowerCase()
      );
    },
    ...
    filteredMatches: state => {
      return Object.keys(state.matches)
        ...
        .sort((a: string, b: string) => {
          return state.matches[a].timestamp < state.matches[b].timestamp
            ? 1
            : -1;
        });
    },
  ...
  mutations: {
    ...
    rankPlayers: state => {
      ...
      state.rankedPlayers.forEach(
        (x: string, index: number) => (state.players[x].rank = index + 1)
      );
    },
    ...
    setActiveMatches: (state, val: MatchMakerMatch[]) => {
  ...
  actions: {
    getPlayers: async ({ state, dispatch }) => {
      if (!Object.keys(state.players).length) await dispatch("fetchPlayers");
      return state.players;
    },
    ...
    loadMatchMaker: async ({ state, commit, dispatch }) => {
      if (!Object.keys(state.matchMaker).length) {
        commit(
          "setMatchMaker",
          new MatchMaker(await dispatch("getPlayers"), 6)
        );
      }
    },
    setActiveMatches: ({ commit }, val: MatchMakerMatch[]) => {
    ...
    removeActiveMatch: ({ state, commit }, val: MatchMakerMatch) => {
    ...
    completeMatch: async ({ state, commit, dispatch }) => {
      if (state.play.callback) state.play.callback();
      const match = {
