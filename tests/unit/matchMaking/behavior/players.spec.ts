...
describe("Match Maker Players Behavior", () => {
    it("player pool init", () => {
    ...
    const expectedPlayers6 = new Player(
        player6.id,
        player6.gamesPlayed,
        player6.playersPlayed,
        player6.isActive
    );
    expectedPlayers6.isActive = false;
    const expectedPlayers7 = new Player(
        player7.id,
        player7.gamesPlayed,
        player7.playersPlayed,
        player7.isActive
    );
    ...
    testPlayerState(
        ...
        [player1, player2, player3, player5, expectedPlayers6, expectedPlayers7]
    );
    ...
    it("update players - 1 match", () => {
        ...
        expect(players.players[0]).toEqual({
            id: "1",
            gamesPlayed: 1,
            playersPlayed: new Set<string>(["2"]),
            isActive: true,
        });
        expect(players.players[1]).toEqual({
            id: "2",
            gamesPlayed: 1,
            playersPlayed: new Set<string>(["1"]),
            isActive: true,
        });
    });
    ...
    it("update players - 3 match", () => {
        ...
        expect(players.players[0]).toEqual({
            id: "1",
            gamesPlayed: 2,
            playersPlayed: new Set<string>(["2", "7"]),
            isActive: true,
        });
        expect(players.players[2]).toEqual({
            id: "3",
            gamesPlayed: 1,
            playersPlayed: new Set<string>(["5"]),
            isActive: true,
        });
        expect(players.players[3]).toEqual({
            id: "5",
            gamesPlayed: 2,
            playersPlayed: new Set<string>(["3", "6"]),
            isActive: true,
        });
        expect(players.players[4]).toEqual({
            id: "6",
            gamesPlayed: 1,
            playersPlayed: new Set<string>(["5"]),
            isActive: false,
        });
        expect(players.players[5]).toEqual({
            id: "7",
            gamesPlayed: 1,
            playersPlayed: new Set<string>(["1"]),
            isActive: false,
